...
main()
{
    ...
    if [ -d "$PYTORCH_SERVER_VIRTUAL_ENV" ]; then
        echo "Python virtual environment ${blu}$PYTORCH_SERVER_VIRTUAL_ENV${end} exists, deactivating ..."
        deactivate
    else
        echo "Python virtual environment ${blu}$PYTORCH_SERVER_VIRTUAL_ENV${end} does not exist"
    fi
}
