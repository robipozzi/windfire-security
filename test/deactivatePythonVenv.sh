...
main()
{
    ...
    if [ -d "$PYTORCH_TEST_VIRTUAL_ENV" ]; then
        echo - "Python virtual environment ${BLU}$PYTORCH_TEST_VIRTUAL_ENV${RESET} exists, deactivating ..."
        deactivate
    else
        echo -e "Python virtual environment ${BLU}$PYTORCH_TEST_VIRTUAL_ENV${RESET} does not exist"
    fi
}
